...
export class TranslationService {
  ...
  constructor(private readonly i18nService: I18nService) {
    ...
    this.translate = new Translate({
      ...
      credentials: {
        ...
        // .env files often escape the newline characters.
        // The .replace() ensures the private key retains its actual line breaks, which Google requires.
  ...
  async dynamicTranslate(dto: TranslationDto): Promise<SuccessResponse<TranslationResponse>> {
    ...
    try {
      ...
      const [res, Metadata] = await this.translate.translate(text, {
        // from: sourceLang,
        // to: targetLang,
        from: 'ar',
        to: 'en',
      ...
      this.logger.log(`Translation succeeded for text "${text}" to ${res}`);
      this.logger.log(`Metadate: ${JSON.stringify(Metadata)}`);
      return successResponse({ result: res }, 'Translation successful', 200);
    } catch (error) {
      handleErrorsAndThrow(error, `Translation failed for text "${text}"`, this.logger);
    }
  }
